...
### Assign stream types based on clump name
```{r}
...
### Violin plot area by stream (repeat)
```{r}
clump_df %>%
  ggplot(aes(x = stream, y = Area)) + geom_violin()
```

### Extract image name from clump name
```{r}
clump_df$img <- str_split_fixed(clump_df$...1, "_Clump", 2)[, 1]
```

### Area by stream scatter, point size by area
```{r fig.width = 10, fig.height = 10}
clump_df %>%
  ggplot(aes(x = stream, y = Area, color = 'TBR1+ CGE')) + geom_point(size = clump_df$Area / 10e4, ) + RotatedAxis()
```

### Boxplot of ncell (number of cells) per stream, point size by area
```{r}
subset_df <- clump_df %>%
...
### Boxplot of area by stream for clumps
```{r}
clump_df %>%
  filter(grepl('_', stream)) %>%
...
# Identify "weird" clusters based on UMAP2
```{r}
...
plot(net, edge.arrow.size=.4, edge.curved=.5)
```
### First make a heatmap from images and cell types
```{r}
jy_all$slice <- NA
